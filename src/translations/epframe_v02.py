...
def solve_system(KSAT, b, n):
    """
    Solve KSAT * x = b using numpy's linear algebra solver
    KSAT and b are Index1Array objects (1-indexed)
    Returns solution vector x as Index1Array (1-indexed), or None if singular
    """
    # Extract coefficient matrix and RHS to 0-indexed numpy arrays
    A_np = np.zeros((n, n))
    b_np = np.zeros(n)
    ...
    # Convert back to 1-indexed array
    x = Index1Array(n)
    for i in range(1, n+1):
        x[i] = x_np[i-1]
